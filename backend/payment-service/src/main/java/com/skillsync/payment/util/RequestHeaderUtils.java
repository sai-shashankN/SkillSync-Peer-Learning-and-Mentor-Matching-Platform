package com.skillsync.payment.util;
...
public final class RequestHeaderUtils {
    ...
    public static boolean hasMentorRole(HttpServletRequest request) {
        return extractRoles(request).stream()
                .anyMatch(role -> "MENTOR".equalsIgnoreCase(role) || "ROLE_MENTOR".equalsIgnoreCase(role));
    }
    ...
    public static void requireMentor(HttpServletRequest request) {
        if (!hasMentorRole(request) && !hasAdminRole(request)) {
            throw new UnauthorizedException("MENTOR role is required");
        }
    }
